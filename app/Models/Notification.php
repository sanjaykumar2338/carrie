...
class Notification extends Model
{
	...
	public function email_notification($notification_config_id='', $notification_type_id='', $notification_id='')
	{
		...
		{
			$placeholderData = $this->acl_notification_settings($notification, $user);
		}
		elseif($tableModel == 'User')
		{
			$placeholderData = $this->user_notification_settings($notification, $user);
		}
		elseif($tableModel == 'Role')
		{
			$placeholderData = $this->role_notification_settings($notification, $user);
		}
		elseif($tableModel == 'Comment')
		{
			$placeholderData = $this->comment_notification_settings($notification, $user);
		}
		elseif($tableModel == 'Configuration')
		{
			$placeholderData = $this->configuration_notification_settings($notification, $user);
		}
		elseif($tableModel == 'Blog')
		{
			$placeholderData = $this->blog_notification_settings($notification, $user);
		}
		elseif($tableModel == 'BlogCategory')
		{
			$placeholderData = $this->blog_category_notification_settings($notification, $user);
		}
		elseif($tableModel == 'BlogTag')
		{
			$placeholderData = $this->blog_tag_notification_settings($notification, $user);
		}
		elseif($tableModel == 'Page')
		{
			$placeholderData = $this->page_notification_settings($notification, $user);
		}
	...
	private function user_notification_settings($notificationObj, $userObj)
	{
		...
		$placeholderData    =   ['User' =>
									[
										'username' => optional($userObj)->first_name,
	...
	{
		$comment = Comment::select('comment')->where('id', '=', $notificationObj->object_id)->with('blog')->first();
