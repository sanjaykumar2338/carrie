					...
					<div class="card-body">
						<div class="row">
                            ...
                            @if($comment->object_type == 1)
                                <p> {{ __('common.comment_s_blog') }} : <a class="fw-bold" href="{{ DzHelper::laraBlogLink($comment->blog_id) }}"> {{ DzHelper::getBlogTitle($comment->object_id) }}</a></p>
                                <p> {{ __('common.submited_on') }} : {{ $comment->created_at }}</p>
                            @else
                                <p> {{ __('common.comment_s_blog') }} : <a class="fw-bold" href="{{ DzHelper::laraBlogLink($comment->blog_id) }}"> {{ DzHelper::getPageTitle($comment->object_id) }}</a></p>
                                <p> {{ __('common.submited_on') }} : {{ $comment->created_at }}</p>
                            @endif
							<div class=" pt-0">
